...
|[!UICONTROL Statistical Methodology]|Choose how experiment results are analyzed. This choice determines the statistical framework, the result metrics, and the decision criteria used for this activity's [!UICONTROL Conversion] goal metric:<ul><li>**[!UICONTROL Bayesian]**: Probability-based decisioning that updates beliefs as data arrives and supports continuous monitoring and smaller sample sizes. Results are expressed as the probability that one variant is better than another, for example "92% probability that Variant B beats Control". [!UICONTROL Bayesian] is best suited for smaller sample sizes, continuous monitoring, stakeholders who prefer probabilities, or when you have prior knowledge from past tests to apply. It is not ideal when you require strict regulatory or compliance sign-off, or need traditional p-values. Reports show [!UICONTROL Chance to Beat Control] and a credible interval instead of the frequentist [!UICONTROL Confidence] column.</li><li>**[!UICONTROL Welch's t-test (Frequentist)]**: The existing, traditional hypothesis-testing methodology. Uses p-values and requires a fixed sample size. Reports show a [!UICONTROL Confidence] percentage and confidence interval.</li></ul>This setting is only available for activities whose primary goal metric is [!UICONTROL Conversion], activities with a [!UICONTROL Revenue] or [!UICONTROL Engagement] primary goal always use [!UICONTROL Welch's t-test]. For more information about how each methodology is calculated and reported, see [Statistical calculations in A/Bn tests](/help/main/c-reports/statistical-methodology/statistical-calculations.md).|
|[!UICONTROL Bayesian Configuration]|Displayed only when [!UICONTROL Bayesian] is selected as the [!UICONTROL Statistical Methodology]. Configure the following options:<ul><li>[!UICONTROL Credible Interval]: Sets the range within which the true effect is expected to fall.</li><li>[!UICONTROL Decision Threshold]: Sets the minimum probability a variant must reach, relative to [!UICONTROL Control], before it can be declared a winner. </li><li>[!UICONTROL Use prior knowledge from past tests?]: Toggle this option on to provide prior knowledge from past performance to inform the [!UICONTROL Bayesian] calculation.</li></ul>|

...
When you select [!UICONTROL Most Viewed by Profile Attribute] or [!UICONTROL Top Sellers by Profile Attribute], select a **[!UICONTROL Profile Attribute]** to group popularity data. [!DNL Target] builds a separate ranked list for each unique value of the selected attribute and serves the appropriate list to each visitor based on their attribute value at delivery time. For example, selecting `profile.country` causes visitors from the United States to receive the most-viewed or top-selling items among US visitors, while visitors from India see India's top picks.

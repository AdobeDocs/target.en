...
|`landing.url`, `landing.protocol`, `landing.query`, and `landing.param`|Similar to that of page, but for the landing page.<P>Works only if you set the `context` > `browser` > `host`.<P>Ensure that you don't set the referring URL to the same domain on the first call of the session. The first call of the session is the search engine referrer (the third-party domain). Subsequent calls can have the same domain. However, ensure that `referringURL` is the previous URL that the user visited during the current session.|
